...
function do_widget( $args ) {
	...
	$widget_args = apply_filters( 'dynamic_sidebar_params', $widget_args );

	if( ! $show_title ) {
		$widget_args[0]['before_title'] = '<h3 class="widgettitle">';
		$widget_args[0]['after_title'] = '</h3>';
	} elseif( is_string( $title ) && strlen( $title ) > 0 ) {
		$instance['title'] = $title;
	}

	ob_start();
	the_widget( $class, $instance, $widget_args[0] );
